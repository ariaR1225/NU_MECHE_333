...
while not has_quit:
    ...
    if (selection == 'a'):
        ...
        Gp = float(input("Enter your desired Kp current gain [recommended: 4.76]: "))
        Gi = float(input("Enter your desired Ki current gain [recommended: 0.32]: "))
        print(f"Sending Kp = {Gp} and Ki = {Gi} to the current controller.")

    elif (selection == 'h'):
        counts_str = ser.read_until(b'\n')
        counts = int(counts_str)
        print(f"The current controller is using Kp = {Gp} and Ki = {Gi}.")
        ...
        traj_step = input("Input step trajectory: ")
        counts = int(counts_str)
        print(f"Encoder counts: {counts}")

    elif (selection == 'n'):
        traj_cubic = input("Input cubic trajectory: ")
        counts_str = ser.read_until(b'\n')
        counts = int(counts_str)
        print(f"Encoder counts: {counts}")
        ...
        status = int(mode_str)
        # change mode number to mode name
        if status == 0:
            mode = "IDLE"
        elif status == 1:
            mode = "PWM"
            ...
            mode = "None"
        print(f"The PIC32 controller mode is currently {mode}.")
